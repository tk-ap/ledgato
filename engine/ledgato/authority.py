...
class AuthorityStore:
    ...
    def _exclusive(self):
        ...
        Same contract as ApprovalStore: grants are file-backed and each process
        holds its own snapshot, so an unguarded issue/revoke can clobber a
        concurrent writer. Revocation is a security control, so a lost
        revocation write is a real authority bug.
        """
    ...
    def _save(self) -> None:
        if not self.path:
            return
        self.path.parent.mkdir(parents=True, exist_ok=True)
        payload = json.dumps([g.to_dict() for g in self._grants.values()], indent=2, sort_keys=True)
