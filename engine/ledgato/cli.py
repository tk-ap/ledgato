...
def _load_state(config: Path, ledger_file: Path, key_dir: Path, difficulty: int = 0):
    signer = Signer()
    if key_dir.exists():
    ...
    ledger = Ledger(signer=signer, path=ledger_file, difficulty=int(difficulty))
...
def cmd_attestation(args) -> int:
    ...
    print(f"[chain] ledger integrity: {'OK' if ok else 'BROKEN ' + str(errs)} ({len(ledger.entries)} entries, pow={ledger.difficulty})")
...
def cmd_ledger(args) -> int:
    ...
    ledger = Ledger(signer=signer, path=Path(args.ledger), difficulty=args.difficulty)
...
def cmd_report(args) -> int:
    """Export a verifiable attestation report, or verify an exported bundle."""
    if args.verify:
        bundle = json.loads(Path(args.verify).read_text())
        res = attest_ops.verify_report(bundle)
        print(f"[report] id={res['report_id']} agent={res['agent']} release={res['release']}")
        print(f"[report] verdict={res['verdict']} entries={res['entries']}")
        if res["verified"]:
            print(f"[report] VERIFIED ✓ · signatures + proof-of-work + chain intact")
            return 0
        for er in res["errors"]:
            print(f"  !! {er}")
        print("[report] NOT VERIFIED — evidence is not authentic")
        return 1

    policies, ledger = _load_state(
        Path(args.config).resolve(), Path(args.ledger), Path(args.keys), args.difficulty
    )
    bundle = attest_ops.export_report(ledger, args.agent, args.release, publisher=ledger.signer.public_key_b64())
    out = Path(args.out)
    out.write_text(json.dumps(bundle, indent=2))
    print(f"[report] exported {args.agent}@{args.release} · {len(bundle['entries'])} entries → {out}")
    print(f"[report] verify anywhere with: ledgato report --verify {out}")
    return 0


def cmd_status(args) -> int:
    """Show ledger status: head, difficulty, integrity."""
    signer = Signer()
    if Path(args.keys).exists():
        signer = Signer.load(Path(args.keys))
    ledger = Ledger(signer=signer, path=Path(args.ledger), difficulty=args.difficulty)
    ...
    ok, errs = ledger.verify_chain()
    print(f"[status] entries={len(ledger.entries)} head={head.hash if head else 'GENESIS'}")
    print(f"[status] difficulty={ledger.difficulty} chain_integrity={'OK' if ok else 'BROKEN'}")
    if not ok:
        for er in errs:
            print(f"  !! {er}")
        return 1
...
def cmd_sync(args) -> int:
    """Reconcile this ledger with a remote node's chain (longest-valid-chain)."""
    policies, ledger = _load_state(
        Path(args.config).resolve(), Path(args.ledger), Path(args.keys), args.difficulty
    )
    node = Node(ledger, node_id=args.self_id or f"node-{args.port}")
    if args.remote:
        import urllib.request

        url = args.remote.rstrip("/") + "/v1/ledger/chain"
        with urllib.request.urlopen(url, timeout=args.timeout) as r:
            remote = json.loads(r.read())
        result = node.reconcile(remote.get("entries", []))
        if result.adopted:
            print(f"[sync] adopted remote chain ({result.remote_len} > {result.local_len}) · {result.reason}")
        else:
            print(f"[sync] kept local chain · {result.reason}")
        print(f"[sync] local now {len(ledger.entries)} entries · verified={ledger.verify_chain()[0]}")
        return 0 if result.adopted else (1 if result.reason == "remote chain invalid" else 0)
    # manual chain reconcile: accept a JSON file with {entries: [...]}
    data = json.loads(Path(args.file).read_text())
    result = node.reconcile(data.get("entries", data if isinstance(data, list) else []))
    print(f"[sync] {result.reason}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--difficulty", type=int, default=DEFAULT_DIFFICULTY, help="ledger proof-of-work difficulty")
    _add_state(s)
    ...
    s.add_argument("--difficulty", type=int, default=0)
    s.set_defaults(func=cmd_ledger)

    s = sub.add_parser("status", help="show ledger status (entries, head, proof-of-work)")
    _add_state(s)
    s.add_argument("--difficulty", type=int, default=DEFAULT_DIFFICULTY)
    s.set_defaults(func=cmd_status)

    s = sub.add_parser("report", help="export or verify a verifiable attestation report")
    s.add_argument("--agent")
    s.add_argument("--release")
    s.add_argument("--out", default="attestation-report.json")
    s.add_argument("--verify", help="verify an exported report bundle (JSON)")
    s.add_argument("--difficulty", type=int, default=DEFAULT_DIFFICULTY)
    _add_state(s)
    s.set_defaults(func=cmd_report)

    s = sub.add_parser("sync", help="reconcile this ledger with a remote node (longest-valid-chain)")
    s.add_argument("--remote", help="remote node base URL (http://host:port)")
    s.add_argument("--file", help="reconcile from a local JSON chain file")
    s.add_argument("--self-id", dest="self_id", help="this node's id")
